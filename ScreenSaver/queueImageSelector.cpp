...
std::string queueImageSelector::getNextImageInQueue (std::vector<std::string> &displayList)
{
	...
	int numFodlersInDir = FileUtils::GetNumFilesInDir(dir);
	...
	return FileUtils::GetRandomDirQuick(newDir);
}
//--------------------------------------------------------------------------------------------------------
vector<string> queueImageSelector::queueFolderList(string path)
{
	...
	vector<string> folderList = CFGUtils::GetFloatValueFromList(path);
	...
	for(int i = 0; i < numFolders; i++)
	{
		...
		do{
			...
			else
			{
				...
				if(seqList[curDir] < FileUtils::GetNumFilesInDir(dir) &&  seqList[curDir] > -1)	
					seqList[curDir] ++;
				else
					seqList[curDir] = 0;
