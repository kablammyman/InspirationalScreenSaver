...
void optionsScreen::draw()
{
	clear(infoImg->getBitmap());
      ...
      //VIC//textprintf(infoImg->getBitmap(), font,  0, 20, makecol(255,255,255), "length:   %d", edittext.length());
      //VIC//textprintf(infoImg->getBitmap(), font,  0, 30, makecol(255,255,255), "capacity: %d", edittext.capacity());
      //VIC//textprintf(infoImg->getBitmap(), font,  0, 40, makecol(255,255,255), "empty?:   %d", edittext.empty());
      if(insert)
...
int menu_class::show_menu(DATAFILE *the_datafile, PIXMAP *bmp, int da_font)
{
	...
	if (the_datafile != NULL && da_font != -1)
	{
		for (int i = 0; i < (int)menuItems.size(); i++)
		{
			...
			if (menuItems[i].second >= 0)
				//VIC//textprintf(bmp, (FONT*)the_datafile[da_font].dat, x + (font_size + (font_size*menuItems[i].first.size())), y + level_y * i, font1_color, "%d", menuItems[i].second);
		}
	}
	else
	{
		for (int i = 0; i < (int)menuItems.size(); i++)
		{
			...
			if (menuItems[i].second >= 0)
				//VIC//textprintf(bmp, font, x + (font_size + (font_size*menuItems[i].first.size())), y + level_y * i, font1_color, "%d", menuItems[i].second);
		}
	}
...
void menu_class::draw(PIXMAP *dest)
{
