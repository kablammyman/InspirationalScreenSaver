...
public:
	...
	void init(int _id, short _x, short _y, short _w, short _h, std::string name1, std::string name2)
	{
		...
		//VIC//rectfill(img1, 0, 0, width, height, makecol(0,0,255));
		//VIC//rectfill(img2, 0, 0, width, height, makecol(255,0,0));
	}
	...
	void draw(PIXMAP *buffer)
	{
		if(isPressed)
		{
			//VIC//draw_sprite(buffer,img2, x, y);
			//VIC//if(isHighlighted)
			//VIC//	rect(buffer, x, y, x+width-1, y+height-1, makecol(100,100,100));
			//VIC//textout_ex(img2, font, text[2].c_str(), 1, 1, makecol(255,255,255), 0);
		}
		else
		{
			//VIC//draw_sprite(buffer,img1, x, y);
			//VIC//if(isHighlighted)
			//VIC//	rect(buffer, x, y, x+width-1, y+height-1, makecol(100,100,100));
